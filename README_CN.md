...
<p align="center">
  <img src="docs/screenshots/login.png" width="32%" alt="登录" />
  <img src="docs/screenshots/lobby.png" width="32%" alt="大厅" />
  <img src="docs/screenshots/game-table.png" width="32%" alt="游戏桌" />
</p>
...
texas-holdem/
├── packages/
│   ├── shared/          # 类型定义、WebSocket 协议、扑克工具
│   ├── client/          # React 单页应用 (Vite)
│   │   ├── components/  # 登录、大厅、等待室、结算弹层
│   │   ├── game/        # 牌桌、玩家座位、扑克牌、筹码堆
│   │   ├── hooks/       # useVoice (LiveKit)
│   │   ├── stores/      # Zustand 游戏状态
│   │   └── lib/         # WebSocket 客户端、音效管理
│   └── server/          # Hono HTTP + WebSocket 服务端
│       ├── ai/          # AI 决策引擎 + 性格配置
│       ├── engine/      # 游戏状态机、手牌评估、底池计算
│       ├── rooms/       # 房间生命周期管理
│       ├── ws/          # WebSocket 消息路由
│       ├── auth/        # JWT 签发/验证
│       └── db/          # SQLite 初始化 + 用户仓库
...
服务端 (Hono + ws)
  ...
  ├── AIPlayerManager → Claude API / 规则引擎
